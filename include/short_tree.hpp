...
template <typename _Tp> struct node {
    ...
    node(self&& _rhs): _v(std::move(_rhs._v)) {}
    node& operator=(const self& _rhs) {
        if (&_rhs == this) return *this;
        _v = _rhs._v;
        return *this;
    }
    node& operator=(self&& _rhs) {
        _v = std::move(_rhs._v);
        return *this;
    }
...
template <typename _Tp> struct short_tree_node : public node<_Tp> {
    ...
    const self* parent() const { return _parent; }
};
...
public:
    ...
    // > friend class disjoint_set<_Key, _Value, _ExtKey, _ExtValue, _Hash, _Alloc>;
...
public:
    ...
    size_t size() const { return _m_impl._node_count; }
    bool empty() const { return _m_impl._node_count == 0; }
    node_type* add(const value_type& _v);
    // node_type* add(const value_type& _v, typename std::enable_if<!std::is_same<value_type, void>::value, int>::type = 0);
    // node_type* add(std::nullptr_t = nullptr, typename std::enable_if<std::is_same<value_type, void>::value, int>::type = 0);
...
// };
template <typename _Tp, typename _Alloc> auto
short_tree<_Tp, _Alloc>::query_header(const node_type* _x) -> const node_type* {
    if (_x == nullptr) return nullptr;
    while (_x->_parent->_parent != _x) {
        _x = _x->_parent;
    }
